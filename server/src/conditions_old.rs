...
fn replace_condition_templates(value: &Value, state: &Value, current_actor: &str) -> Value {
    match value {
        Value::String(s) => {
        ...
        Value::Object(obj) => {
            ...
            for (k, v) in obj {
                new_obj.insert(k.clone(), replace_condition_templates(v, state, current_actor));
            }
            Value::Object(new_obj)
        }
        Value::Array(arr) => {
            Value::Array(arr.iter().map(|v| replace_condition_templates(v, state, current_actor)).collect())
        }
        _ => value.clone()
    }
}
...
pub fn evaluate_condition(
    ...
    let processed_with = replace_condition_templates(&with_value, state, current_actor);

...
pub fn load_initial_state(bundle: &Bundle) -> Value {
    ...
    if let Some(arr) = bundle.zones.as_array() {
        for zone in arr {
            ...
            // clone contents value or use "empty" if not provided
            let contents = zone
                .get("contents")
                .cloned()
...
fn apply_move_entity(_bundle: &Bundle, state: &mut Value, spec: &Value, action: &Value) -> Value {
    // capture the current actor as owned string
    ...
    // rotate turn
    let next_player = if let Some(players) = state["players"].as_array() {
        players
            .iter()
            .position(|p| p["id"].as_str() == Some(actor.as_str()))
            .and_then(|idx| players[(idx + 1) % players.len()]["id"].as_str())
            .map(|s| s.to_string())
    } else {
        None
    };

...
Entities are all game components: cards, tokens, pieces, etc.

```yaml
- id: string                    # Unique identifier (required)
  type: string                  # Category for rules (required)
  abstract: boolean             # Base type only? Can't instantiate
  extends: entity_id            # Inherit from another entity
  singleton: boolean            # Only one can exist?
  
  # Properties define entity data
  props:                        # All gameplay properties
    # Common properties
    name: string                # Display name
    description: string         # Flavor text
    category: string            # Grouping type
    cost: map | integer         # Resource cost
    value: integer              # Numeric value
    points: integer             # Victory points
    
    # Parameterized properties (for inheritance)
    property: {param}           # Filled by child entities
    
    # Any custom properties for your game
    [key]: any                  # Game-specific data
    
  # State management
  state:                        # Mutable properties
    tapped: boolean             # Is it exhausted?
    damaged: integer            # Damage counters
    faceDown: boolean           # Hidden information?
    owner: player_id            # Current controller
    [key]: any                  # Game-specific state
  ...
  ui:
    ...
    shape: string               # "square" | "circle" | "hex" | custom
    label: string               # Short text label
    tooltip: string             # Hover text
    style: map                  # CSS-like properties
    
  # Behavioral flags
  flags:
    stackable: boolean          # Can multiple exist in same space?
    tradeable: boolean          # Can be given to others?
    destructible: boolean       # Can be removed from game?
    rotateable: boolean         # Orientation matters?
    flippable: boolean          # Has two sides?
    
  # Relationships
  relationships:
    conflicts: [entity_id]      # Can't coexist with
    requires: [entity_id]       # Needs these to function
    replaces: [entity_id]       # Upgrades from
    synergy: [entity_id]        # Works well with
    
  # Abilities (for cards/units)
  abilities:
    - id: string
      trigger: string           # When does it activate?
      cost: map                 # Activation cost
